<template>
  <div>
    ...
    <el-dialog title="成员变更" :visible.sync="dialogVisible" width="50%">
      <el-form>
        <el-form-item label="添加成员:">
          <el-checkbox-group v-model="form.addMember">
            <el-checkbox
              v-for="item in addMembers"
              :key="item.id"
              :label="item.id"
              name="addMember"
...
export default {
  ...
  methods: {
    ...
    handleAddMember: function () {
      Promise.all([
        projectAPI.getFriends(this.uid, this.projectid),
        userAPI.getOthers(this.uid)
      ]).then(data => {
        this.friends = data[0]
        this.members = data[1]
